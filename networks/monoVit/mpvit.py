...
from torch.nn.modules.batchnorm import _BatchNorm  # 原生PyTorch BatchNorm基类
...
# -------------------------- 替换mmcv的build_norm_layer --------------------------
def build_norm_layer(norm_cfg, num_features):
    """
    替换mmcv的build_norm_layer，仅保留BN/SyncBN支持（MPViT核心用BN）
    :param norm_cfg: 归一化配置字典，如 dict(type="BN")
    :param num_features: 归一化层的通道数
    :return: (name, norm_layer)，与mmcv接口对齐
    """
    if norm_cfg["type"] == "BN":
        ...
        raise NotImplementedError(f"仅支持 BN/SyncBN，暂不支持 {norm_cfg['type']}")
...
class MPViT(nn.Module):
    ...
    def __init__(
        ...
        self.mhca_stages = nn.ModuleList(
            [
                MHCA_stage(
                    embed_dims[idx],
                    embed_dims[idx + 1] if not (idx + 1) == self.num_stages else embed_dims[idx],
                    num_layers[idx],
                    num_heads[idx],
                    mlp_ratios[idx],
                    num_path[idx],
    ...
    def init_weights(self, pretrained=None):
        """
        替换mmcv的load_checkpoint，用原生PyTorch加载权重
        :param pretrained: 预训练权重路径（str）或None
        """
        def _init_weights(m):
        ...
        if isinstance(pretrained, str):
            ...
            # 兼容dict中的model键（如原权重是 dict(model=xxx)）
...
def mpvit_xsmall(**kwargs):
    ...
    # 原生PyTorch加载权重（替换mmcv的load_state_dict）
    try:
        checkpoint = torch.load('./ckpt/mpvit_xsmall.pth', map_location="cpu")['model']
        model.load_state_dict(checkpoint, strict=False)
        del checkpoint
    except FileNotFoundError:
        print("警告：未找到 mpvit_xsmall.pth 权重文件，将使用随机初始化权重")
    model.default_cfg = _cfg_mpvit()
    return model
...
def mpvit_small(**kwargs):
    ...
    # 原生PyTorch加载权重
    try:
        checkpoint = torch.load('./ckpt/mpvit_small.pth', map_location="cpu")['model']
        model.load_state_dict(checkpoint, strict=False)
        del checkpoint
    except FileNotFoundError:
        print("警告：未找到 mpvit_small.pth 权重文件，将使用随机初始化权重")
    model.default_cfg = _cfg_mpvit()
    return model
...
if __name__ == '__main__':
    ...
    model = mpvit_xsmall()
    x = torch.randn(1, 3, 192, 640)  # Lite-Mono 输入尺寸
    outs = model(x)
    print(f"MPViT 输出特征数：{len(outs)}")
    print(f"最后一层特征尺寸：{outs[-1].shape}")
    print("✅ MPViT 加载&推理成功，无 mmcv._ext 错误！")